...
val_parser.add_argument('--patch_size', nargs='+', type=int, default=[32,32,32],
                    help='Patch size using to validate model')
...
if __name__ == '__main__':
    ...
    normalize_transform = NormalizeIntensityd(keys=['image'], subtrahend=subtrahend, divisor=divisor, channel_wise=True)
    ...
    if args.axis == 'z':
        rotate_transform = Rotated(keys=["image", "label"], angle=(rotate_angle, 0, 0), keep_size=False, mode=("bilinear", "nearest"))
    elif args.axis == 'y':
        rotate_transform = Rotated(keys=["image", "label"], angle=(0, rotate_angle, 0), keep_size=False, mode=("bilinear", "nearest"))
    elif args.axis == 'x':
        rotate_transform = Rotated(keys=["image", "label"], angle=(0, 0, rotate_angle), keep_size=False, mode=("bilinear", "nearest"))
    elif args.axis == 'all':
        rotate_transform = Rotated(keys=["image", "label"], angle=(rotate_angle, rotate_angle, rotate_angle), keep_size=False, mode=("bilinear", "nearest"))
    ...
    image_saver = NiftiSaver(output_dir=args.output_dir, output_postfix=f'{args.rotate_angle}_degree_{args.axis}_axis', resample=False, data_root_dir=args.test_path[:-13])
    label_saver = NiftiSaver(output_dir=args.output_dir, output_postfix=f'{args.rotate_angle}_degree_{args.axis}_axis', resample=False, data_root_dir=args.test_path[:-13], output_dtype=np.uint8)
    pred_saver = NiftiSaver(output_dir=args.output_dir, output_postfix=f'{args.rotate_angle}_degree_{args.axis}_axis_prediction', resample=False, data_root_dir=args.test_path[:-13], output_dtype=np.uint8)
    ...
    with torch.no_grad():
        outputs = sliding_window_inference(image,
    ...
    print({'Validation Dice Score CSF': dice_scores[0],
